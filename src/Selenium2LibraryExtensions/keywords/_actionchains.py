...
class _ActionChainsKeywords:
    ...
    def chain_drag_and_drop_with_offset(self, source, target, xoffset, yoffset):
        """Drags element identified with locator by movement
        ...
        element = self.s2l._element_find(source,True,True)
        target =  self.s2l._element_find(target,True,True)
        self.__lazy_init_action_chains().click_and_hold(element).move_to_element_with_offset(
                target, xoffset, yoffset).release()
    ...
    def chain_move_to_element_with_offset(self, target, xoffset, yoffset):
        element = self.s2l._element_find(target,True,True)
        self.__lazy_init_action_chains().move_to_element_with_offset(element,
                xoffset, yoffset)
    ...
    def chain_sleep(self, time_, reason=None):
        ...
        """
        self.__lazy_init_action_chains()._actions.append(lambda:
                BuiltIn().sleep(time_,reason))
